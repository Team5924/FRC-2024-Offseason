...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    NamedCommands.registerCommand("setIntakeStateFloor", new SetIntakeState(intake, elevator, feeder, IntakeState.FLOOR, true));
    NamedCommands.registerCommand("setIntakeStateFloorWithoutOtherStuff", new SetIntakeState(intake, elevator, feeder, IntakeState.FLOOR, false));
    NamedCommands.registerCommand("setIntakeStateHover", new SetIntakeState(intake, elevator, feeder, IntakeState.HOVER, true));
    NamedCommands.registerCommand("setIntakeStateRetract", new SetIntakeState(intake, elevator, feeder, IntakeState.RETRACT, true));
   ...
   NamedCommands.registerCommand("deployIntake", new ParallelCommandGroup(
    new SetIntakeState(intake, elevator, feeder, IntakeState.FLOOR, true),
   ...
   NamedCommands.registerCommand("deployIntakeWithoutPrepare", new ParallelCommandGroup(
    ...
    autoModeChooser.addOption("2 Note Pickup Mid", "2 Note Pickup Mid");
  ...
  private void configureButtonBindings() {
    ...
    operatorController.a().onTrue(
      new SetShooterState(shooter, ShooterState.SPIT)
    ).onFalse(
      new SetShooterState(shooter, ShooterState.OFF)
    );
    ...
    operatorController.leftTrigger().whileTrue(new ParallelCommandGroup(
      new SetIntakeState(intake, elevator, feeder, IntakeState.EJECT, true),
      new SetWristAndElevatorState(elevator, WristAndElevatorState.INTAKE)
    )).onFalse(new ParallelCommandGroup(
      new SetIntakeState(intake, elevator, feeder, intake.getStateBeforeEject(), true),
      new SetFeederState(feeder, FeederState.MANUAL),
      new SetWristAndElevatorState(elevator, WristAndElevatorState.AIM_LOW)
    ));
    ...
    operatorController.leftBumper().onTrue(new SetIntakeState(intake, elevator, feeder, IntakeState.RETRACT, true));
    // Triggers elevator and wrist state change to INTAKE
    operatorController.rightBumper().onTrue(new SetIntakeState(intake, elevator, feeder, IntakeState.FLOOR, true));
